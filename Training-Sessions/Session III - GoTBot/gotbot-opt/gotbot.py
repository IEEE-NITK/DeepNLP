...
import random
import os
from gotpreprocess import getID, sen2enco, load_data, filter_line
...
vocab, training_samples, test_samples = load_data(PATH='./')
...
def createBatch(samples):
  ...
  for i in range(batch_size):
    ...
    deco_seqs.append([getID('<go>',word2id)] + sample[1] + [getID('<eos>',word2id)])  # Add the <go> and <eos> tokens
    target_seqs.append(deco_seqs[-1][1:])  # Same as decoder, but shifted to the left (ignore the <go>)
    enco_seqs[i] = [getID('<pad>',word2id)] * (MAX_INPUT_LENGTH  - len(enco_seqs[i])) + enco_seqs[i]
    init_weights.append([1.0] * len(target_seqs[i]) + [0.0] * (MAX_INPUT_LENGTH + 2 - len(target_seqs[i])))
    deco_seqs[i] = deco_seqs[i] + [getID('<pad>',word2id)] * (MAX_INPUT_LENGTH + 2 - len(deco_seqs[i]))
    target_seqs[i]  = target_seqs[i]  + [getID('<pad>',word2id)] * (MAX_INPUT_LENGTH + 2 - len(target_seqs[i]))
...
network_cell = tf.nn.rnn_cell.DropoutWrapper(
...
if TEST_MODE:
  ...
  while True:
    ...
    for wordId in rep_seq:
      if wordId == getID('<eos>',word2id):  # End of generated sentence
        break
      elif wordId != getID('<pad>', word2id) and wordId != getID('<go>',word2id):
